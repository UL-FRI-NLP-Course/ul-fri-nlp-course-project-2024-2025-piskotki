...
def main():
    ...
    parser.add_argument('--output', default='comparison_results.csv', help='Output file name (CSV only, will be placed in /data/results)')
    ...
    output_path = os.path.join(result_dir, timestamped_output)
    ...
    plt.ylim(0, 1.1)
    plt.tight_layout()
    plt.savefig(os.path.join(result_dir, f'metrics_summary_{timestamp_ms}.png'))

    # Save histogram of overall scores
    plt.figure(figsize=(10, 6))
    ...
    plt.savefig(os.path.join(result_dir, f'score_distribution_{timestamp_ms}.png'))

    print(f"Results saved to {output_path}, metrics_summary_{timestamp_ms}.png, and score_distribution_{timestamp_ms}.png")
