...
import numpy as np
import faiss
from wiki_querying import search
...
RAG_TOP_K = 1             # how many chunks to retrieve
...
def iterative_rag(query: str, top_k=RAG_TOP_K):
    ...
    for idx in chosen:
        ...
        out = generator(prompt, max_new_tokens=128, do_sample=False)[0]
        ans = out["generated_text"].split("Answer")[-1].strip(" :\n")
        partials.append(ans)
    ...
    fused = generator(fuse_prompt, max_new_tokens=256, do_sample=False)[0]
